...
<html>
  <head>
	...
	<h1>LISTE EMPLOYES ABSENTS (COORDINATION)</h1>
  </head>
<body>
	<div class="content">
								<!-- Page Heading Start -->
  ...
  <?php
	...
	<?php
	if($_SESSION['NIV']==0 OR $_SESSION['NIV']==4 OR $_SESSION['NIV']==5 ){
	?>
			...
			<table id="datatables-1" class="table table-striped table-bordered" cellspacing="0" width="100%">
				...
				if($resultat){
					...
					while( $auto = mysql_fetch_array($resultat)){
						...
						$sql1 = "SELECT * FROM `source` WHERE smat = '".$auto['mata']."'";

						$resultat1 = mysql_query( $sql1, $cnx ) ;
						
						if(!empty($auto['remplacement'])){
							
							while( $auto1 = mysql_fetch_array($resultat1)){

							$sql2 = "SELECT * FROM `source` WHERE smat = '".$auto['remplacement']."'";

							$resultat2 = mysql_query( $sql2, $cnx ) ;


							while( $auto2 = mysql_fetch_array($resultat2)) {
								?>
								<tr>
									...
									<td> <?php echo($auto2['snom'].' '.$auto2['sprenom'].'('.$auto2['smat'].')') ?></td>
									<td> <?php echo($auto['obsCP']) ?></td>
									<td><a class="btn btn-success"
										   href="enregistrerCommCoor.php?id=<?php echo($auto['idAbscence']) ?>&nom=<?php echo($auto1['snom'] . ' ' .$auto1['sprenom'].'_'.$auto1['smat']) ?>"
										   role="button"> valider</a></td>
								</tr>
								<?php
							}
						}
